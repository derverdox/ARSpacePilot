...
public class Fire : MonoBehaviour
{
    ...
    void getVariables()
    {
        energyCost = gameController.playerMaxEnergy * 0.01f * gameController.energyCostInPercent; //Gettet die Variable aus dem GameController
    }
    ...
    void shoot()
    {
        ...
        if (Input.GetKeyDown(KeyCode.Space)) //Wenn der Input eine Spacebar ist
        {
            if(playerEnergy >= energyCost) //Hat der Spieler genug Energy?
            {
                ...
                GameObject.FindGameObjectWithTag("GameController").GetComponent<ParticleController>().summonLaserSound(transform.position, transform.rotation);
            }
        }
    }
    ...
    {
        Vector3 spawnPos = new Vector3(transform.position.x+distX, transform.position.y + distY, transform.position.z); //Erstellt die Spawnposition der Bullet
